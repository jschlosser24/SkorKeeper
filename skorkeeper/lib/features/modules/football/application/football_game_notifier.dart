...
import '../domain/football_module.dart';

part 'football_game_notifier.g.dart';

@riverpod
class FootballGameNotifier extends _$FootballGameNotifier {
  final FootballModule _module = const FootballModule();

  @override
  Future<SportGameState> build(int sessionId) async {
    final current = await loadSportState(ref, sessionId);
    ref
        .read(sportTimerNotifierProvider(sessionId).notifier)
        .attach(
          onTick: _handleTick,
  ...
  Future<void> dispatch(ScoreAction action) async {
    ...
    final timerNotifier = ref.read(
      sportTimerNotifierProvider(sessionId).notifier,
    );
  ...
  Future<void> editScore(int homeScore, int awayScore) {
    return dispatch(
      SportScoreEdited(homeScore: homeScore, awayScore: awayScore),
    );
  }
