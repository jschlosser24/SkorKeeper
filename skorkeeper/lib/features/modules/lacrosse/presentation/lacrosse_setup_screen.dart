...
import '../domain/lacrosse_state.dart';

class LacrosseSetupScreen extends ConsumerStatefulWidget {
  const LacrosseSetupScreen({super.key});

  @override
  ConsumerState<LacrosseSetupScreen> createState() => _LacrosseSetupScreenState();
}

class _LacrosseSetupScreenState extends ConsumerState<LacrosseSetupScreen> {
  ...
  final _periodMinutesController = TextEditingController(
    text: '${LacrosseStateHelper.defaultPeriodDurationSeconds ~/ 60}',
  );
  String _format = 'full';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Lacrosse Setup')),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: const [
                ...
                DropdownMenuItem(value: 'short', child: Text('Short Game')),
  ...
  Future<void> _startGame() async {
    ...
    final state = SportGameState(
      ...
      homeTeam: SportTeam(
        ...
        stats: LacrosseStateHelper.initialTeamStats(),
      ),
      awayTeam: SportTeam(
        id: 'away',
    ...
    final sessionId = await createSportSession(
      ref,
      gameType: SportType.lacrosse.gameTypeId,
    ...
    if (mounted) {
      context.go('/sports/lacrosse/game?sessionId=$sessionId');
    }
  }
  ...
  int _periodMinutes() {
    return int.tryParse(_periodMinutesController.text.trim()) ??
        (LacrosseStateHelper.defaultPeriodDurationSeconds ~/ 60);
  }
}
