...
part 'hockey_game_notifier.g.dart';

@riverpod
class HockeyGameNotifier extends _$HockeyGameNotifier {
  final HockeyModule _module = const HockeyModule();
  ...
  Future<void> _handleTick() async {
    ...
    final penalties = SportModuleUtils
        .mapList(sportSpecific['activePenalties'])
        .where(
          (penalty) =>
              current.elapsedSeconds + 1 -
                  SportModuleUtils.asInt(penalty['startedAtSeconds']) <
              SportModuleUtils.asInt(penalty['durationMinutes']) * 60,
        )
        .toList();
